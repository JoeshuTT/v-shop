<template>
  <div class="container">
      ...
      <div v-if="list.length" class="list">
        <div v-for="item in list" :key="item.id" class="list-item">
          <div class="list-item-hd" :style="[{ background: tabList[active] ? tabList[active].bg : '' }]">
            ...
            <div class="list-item-desc van-ellipsis">{{ item.moneyHreshold | thresholdTitle }}</div>
          </div>
          <div class="list-item-bd">
            <div class="list-item-bd-inner">
              <div class="list-item-bd-title van-ellipsis">{{ item.name }}</div>
              <div class="list-item-bd-txt">{{ item.dateStart | dateFormat }}至{{ item.dateEnd | dateFormat }}</div>
            </div>
    ...
    <!-- 兑换优惠口令弹窗 -->
    <ExchangeCoupon v-model="exchangeShow" @success="onExchangeSuccess" />
  </div>
</template>
...
export default {
  ...
  data() {
    return {
      ...
      listLoading: true,
      listEmptyText: '暂无优惠券',
      listEmptyImage: require('@/assets/images/empty/coupon.png'),
