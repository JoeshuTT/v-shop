  ...
  <van-pull-refresh v-model="isLoading" @refresh="onRefresh">
    <div class="container">
      <div class="header">
        ...
        <OrderSteps v-model="stepsPopupShow" :list="logList" />
      </div>
      ...
      <div class="section">
            ...
            <div class="list-item-content">
              ...
              <div class="list-item-bottom">
                <div class="list-item-price text-brand-color">
                  ...
                  <span class="list-item-price-integer">{{ item.amountSingle | decimalFormat }}</span>
                </div>
        ...
        <div class="subtotal">
          ...
          <span class="subtotal-price">
            ...
            <span class="subtotal-price-integer">{{ orderInfo.amount | decimalFormat }}</span>
          </span>
        </div>
      </div>
      <!-- 备注 -->
      ...
      <div class="section">
        <div class="amount">
          ...
          <div class="amount-bd">¥ {{ orderInfo.amount | decimalFormat }}</div>
        </div>
        <div v-if="orderInfo.isNeedLogistics" class="amount">
          <div class="amount-hd">运费</div>
          <div class="amount-bd">+ {{ orderInfo.amountLogistics | decimalFormat }}</div>
        </div>
      ...
      <div class="action-bar-wrap">
        <div class="action-bar">
          ...
          <template v-if="orderInfo.status === 0">
            <div class="action-bar-hd">
              ...
              <div class="action-bar-price">
                ...
                <span class="action-bar-price-integer">{{ orderInfo.amountReal | decimalFormat }}</span>
              </div>
            </div>
          ...
          <template v-if="orderInfo.status === 3">
            <van-button class="action-bar-btn" round @click.stop="onOrderReputation(orderInfo.id)">评价</van-button>
          </template>
          <!-- ▲ 操作按钮组 -->
        </div>
      </div>
      <!-- 评价弹层 -->
      <OrderRate v-model="ratePopupShow" :goods="goods" :order-info="orderInfo" @success="onRateSuccess" />
    </div>
  </van-pull-refresh>
</template>
...
export default {
  ...
  components: { Price, OrderSteps, OrderRate },
  data() {
    return {
      isLoading: false,
      orderInfo: {},
  ...
  methods: {
    ...
    onStepsOpen() {
      this.stepsPopupShow = true;
    },
    onOrderReputation() {
      this.ratePopupShow = true;
    },
    onRateSuccess() {
      this.onRefresh();
    },
    ...
    onOrderPay() {
      this.$toast({ message: '未开放：收银台', duration: 1500 });
    },
    onConcatService() {
      this.$toast('未开放：客服');
    },
    onOrderDelivery(orderId) {
      this.$dialog
        .confirm({
          title: '提示',
          message: '确认您已收到商品？',
        ...
        .then(() => {
          ...
          API_ORDER.orderDelivery({ orderId }).then((res) => {
    ...
    onCopy(text) {
      setClipboardData({ data: text }).then((res) => {
        this.$toast({
          message: '复制成功',
          duration: 1500,
        });
    ...
    getDetail() {
      API_ORDER.orderDetail({ orderNumber: this.$route.query.orderNumber })
        .then((res) => {
          const { orderInfo, logs, goods, logistics = {} } = res.data;
          ...
          // 待支付的订单
          if (this.orderInfo.status === 0 && orderInfo.dateClose) {
            const end = dayjs(orderInfo.dateClose);
            const now = dayjs();
            this.closeTime = end.diff(now);
          }
        })
        .finally(() => {
